...
test("temporary scalable map previews and metadata registry are deleted", async () => {
  for (const filename of [
    "miniprogram/services/venue-map-preview.ts",
    "miniprogram/dev/venue-map-preview-fixture.ts",
    "miniprogram/dev/poi-search-preview.ts",
  ]) assert.equal(existsSync(filename), false, filename);
  ...
  ].map((filename) => readFile(filename, "utf8")))).join("\n");
  assert.doesNotMatch(sourceText, /VenueDistrictSidecar|venue-map-preview|poi-search-preview|DEV_ONLY_POI_SEARCH_PREVIEW|previewPoiSearchCapability/);
});
