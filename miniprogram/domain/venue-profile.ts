...
function published(value: unknown, path: string): PublishedVenueProfile {
  ...
  return {
    ...
    facilities: arrayAt(o.facilities, `${path}.facilities`).map((item, index) => { const p = `${path}.facilities[${index}]`; const f = exactObject(item, ["code", "name", "sort_order"], p); return { code: enumAt(f.code, FACILITY_CODES, `${p}.code`), name: stringAt(f.name, `${p}.name`), sortOrder: integerAt(f.sort_order, `${p}.sort_order`) }; }),
    pitchSizes: arrayAt(o.pitch_sizes, `${path}.pitch_sizes`).map((item, index) => enumAt(item, PITCH_SIZES, `${path}.pitch_sizes[${index}]`)),
    ...
    availabilityTarget: { enabled: booleanAt(target.enabled, `${path}.availability_target.enabled`), label: enumAt(target.label, ["查看可订时段"] as const, `${path}.availability_target.label`), path: nullable(target.path, `${path}.availability_target.path`, stringAt) },
  };
}
...
export function decodeVenueProfileUploadIntent(value: unknown): VenueProfileUploadIntent {
  ...
  const requiredHeaders: Record<string, string> = {}; for (const [key, header] of Object.entries(headers)) requiredHeaders[key] = stringAt(header, `$.required_headers.${key}`, true);
  ...
  return { imageId: uuidAt(o.image_id, "$.image_id"), objectKey: stringAt(o.object_key, "$.object_key"), signedPutUrl: httpsUrlAt(o.signed_put_url, "$.signed_put_url"), requiredHeaders, maximumBytes: 10485760, acceptedMimeTypes };
}
