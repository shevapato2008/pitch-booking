...
interface DraftPitch extends Omit<ConfiguredPitch, "id"> { readonly id?: string; readonly clientRef?: string; readonly nameSource?: string; readonly draftStatus?: string }
interface Editor {
...
Page({
  ...
  async loadConfiguration() {
    ...
    try {
      ...
      const pitches = value.pitches.map((pitch) => ({ ...pitch, nameSource: pitch.customName ? "自定义名称" : "系统生成名称" }));
  ...
  onOpenAdd() { if (this.saveInFlight || this.data.mode === "error" || this.data.mode === "loading") return; this.openEditor(undefined); },
  onPitchTap(event: DatasetEvent) { const pitchId = event.currentTarget?.dataset?.pitchId; if (typeof pitchId !== "string" || this.saveInFlight) return; const pitch = this.data.pitches.find((item: DraftPitch) => item.id === pitchId || item.clientRef === pitchId); if (pitch) this.openEditor(pitch); },
  ...
  onCompleteEditor() {
    ...
    if ((editor.customInput && !this.data.isDraftPlayersValid) || (customName?.length ?? 0) > 30) { this.setData({ editor: { ...editor, fieldError: "场地名称需为 1–30 个字符" } }); return; }
    ...
    if (!editor.pitchId && !editor.clientRef) {
      const ref = clientRef(); const next: DraftPitch = { clientRef: ref, customName, systemName: "", displayName: customName || `新建的 ${playersPerSide} 人制场地`, playersPerSide, sequence: 1, status: "ACTIVE", capabilities: draftCapabilities(), nameSource: customName ? "自定义名称" : "保存后生成正式名称", draftStatus: "ACTIVE · 待保存" };
    ...
    const target = this.data.pitches.find((pitch: DraftPitch) => pitch.id === editor.pitchId || pitch.clientRef === editor.clientRef); if (!target) return;
    const updated = { ...target, customName, displayName: customName || target.systemName || target.displayName, playersPerSide, draftStatus: `${target.status} · 待保存` };
  ...
  onBack() { if (this.saveInFlight) return; if (this.data.changes.length) this.setData({ editor: null, dialog: { title: "放弃本次修改？", message: "尚未保存的场地变更将不会生效。", confirmLabel: "放弃修改", cancelLabel: "继续编辑" }, isSheetOpen: true }); else void wx.navigateBack(); },
  ...
  async runSave(attempt: SavePitchConfigurationAttempt) {
    ...
    } finally { this.saveInFlight = false; this.setData({ duplicateSaveDisabled: false }); }
  },
  applySaved(saved: PitchConfiguration) { const pitches = saved.pitches.map((pitch) => ({ ...pitch, nameSource: pitch.customName ? "自定义名称" : "系统生成名称" })); this.setData({ venueName: saved.venue.name, configurationVersion: saved.configurationVersion, baselinePitchCount: pitches.length, pitches, changes: [], configuredCount: pitches.length, mode: pitches.some(({ status }) => status === "ACTIVE") ? "list" : pitches.length ? "inactive-only" : "empty", statusMessage: "场地配置已保存", bannerKind: "success", pageAction: { label: "保存更改", disabled: true } }); },
});
