...
emit({event:'start',pid:process.pid,output,baselineLogFiles:offsets.size,logsOnly});
function sample(){
  samples++;
  if(!logsOnly){
  ...
  const count=metrics['kern.num_files']; min=Math.min(min,count);max=Math.max(max,count);
