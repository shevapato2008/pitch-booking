...
const renderAttendanceRecord = (detail: AttendanceRegistrationDetail): string => {
  ...
  return `${renderAttendanceFeedback()}<header class="detail-heading"><div><p class="eyebrow">报名状态 ${escapeHtml(detail.registration_status)}</p><h2>${escapeHtml(detail.player_display_name)}</h2><p>意向位置 ${escapeHtml(positionLabel(detail.intended_position))} · 当前版本 v${detail.version}</p></div><span class="status-badge status-badge--${attendanceStatusClass(detail.attendance_status)}">${attendanceStatusLabel(detail.attendance_status)}</span></header><div class="attendance-detail-grid"><div class="content-stack"><article class="panel panel--padded"><div class="section-heading"><div><p class="eyebrow">Registration</p><h3>报名与球局</h3></div></div><dl class="facts facts--three"><div><dt>报名编号</dt><dd class="mono">${escapeHtml(detail.registration_id)}</dd></div><div><dt>报名状态</dt><dd>${escapeHtml(detail.registration_status)}</dd></div><div><dt>本场称呼</dt><dd>${escapeHtml(detail.player_display_name)}</dd></div><div><dt>意向位置</dt><dd>${escapeHtml(positionLabel(detail.intended_position))}</dd></div><div><dt>球局</dt><dd>${escapeHtml(detail.game_name)}</dd></div><div><dt>场地</dt><dd>${escapeHtml(detail.venue_name)} · ${escapeHtml(detail.pitch_name)}</dd></div><div class="facts__wide"><dt>时间</dt><dd>${escapeHtml(formatTime(detail.starts_at))} — ${escapeHtml(formatTime(detail.ends_at))}</dd></div></dl></article><article class="panel panel--padded"><div class="section-heading"><div><p class="eyebrow">Audit source</p><h3>原始到场记录</h3></div><span class="status-badge status-badge--${attendanceStatusClass(detail.original_attendance_status)}">${attendanceStatusLabel(detail.original_attendance_status)}</span></div><dl class="facts facts--three"><div><dt>原始结果</dt><dd>${attendanceStatusLabel(detail.original_attendance_status)}</dd></div><div><dt>记录时间</dt><dd>${escapeHtml(formatTime(detail.attendance_recorded_at))}</dd></div><div><dt>当前有效结果</dt><dd>${attendanceStatusLabel(detail.attendance_status)}</dd></div></dl><p class="immutability-note"><span aria-hidden="true">i</span>原始记录会永久保留；平台纠正只追加审计记录并更新当前有效结果。</p></article>${renderAttendanceHistory(detail)}</div><aside class="panel panel--padded correction-panel" tabindex="-1"><p class="eyebrow">Correction</p><h3>纠正后的结果</h3>${target ? `<div class="status-transition"><span class="status-chip status-chip--${attendanceStatusClass(detail.attendance_status)}">${attendanceStatusLabel(detail.attendance_status)}</span><span class="status-transition__arrow" aria-hidden="true">→</span><span class="status-chip status-chip--${attendanceStatusClass(target)}">${attendanceStatusLabel(target)}</span></div><label class="field-label" for="correction-reason">纠正原因 <span class="required">*</span></label><textarea class="reason-input${attendance.state.reasonError ? " has-error" : ""}" id="correction-reason" data-action="attendance-reason-input" aria-describedby="reason-help reason-error" placeholder="说明核验来源与纠正依据" ${locked ? "disabled" : ""}>${escapeHtml(attendance.state.reason)}</textarea><p class="field-help" id="reason-help">原因会写入平台审计记录；玩家和队长只看到纠正后的状态与时间。</p><p class="field-error${attendance.state.reasonError ? " is-visible" : ""}" id="reason-error" role="alert">${escapeHtml(attendance.state.reasonError ?? "")}</p><button class="button button--danger correction-panel__submit" data-action="prepare-attendance-correction" type="button" ${locked ? "disabled" : ""}>${attendance.state.submitting ? "正在确认…" : detail.corrections.length ? "再次纠正" : "发起纠正"}</button>` : `<div class="ineligible-note"><strong>暂不可纠正</strong><p>${escapeHtml(blockedReasonLabel(detail))}</p></div>`}</aside></div>`;
};
