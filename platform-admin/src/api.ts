...
export class PlatformApi {
  ...
  private async request<T>(
    ...
    return (await this.requestWithStatus<T>(path, init, sessionRequest, decoder)).body;
  }
  ...
  private async requestWithStatus<T>(
    ...
    if (response.status === 204) {
      return {
        ...
        body: decoder ? decoder(undefined) : undefined as T,
      };
    }
    let value: unknown;
    try {
    ...
    return {
      status: response.status,
      body: decoder ? decoder(value) : value as T,
    };
  }
}
