...
test("production login disables credential persistence and wires foreground expiry checks", () => {
  ...
  expect(source).toMatch(/GameReportResolutionController/);
  expect(source).toMatch(/gameReportResolutionVisible/);
  expect(source).toMatch(/data-action="open-game-reports"/);
  expect(source).toMatch(/data-action="filter-game-reports"/);
  expect(source).toMatch(/data-action="select-game-report"/);
  expect(source).toMatch(/data-action="refresh-game-reports"/);
  expect(source).toMatch(/data-action="load-more-game-reports"/);
  expect(source).toMatch(/data-action="choose-game-report-outcome"/);
  expect(source).toMatch(/data-action="prepare-game-report-resolution"/);
  expect(source).toMatch(/data-action="cancel-game-report-resolution"/);
  expect(source).toMatch(/data-action="confirm-game-report-resolution"/);
