...
Payment: PREPAY_CREATED  → CONFIRMING | SUCCESS | CLOSED | UNKNOWN
```
...
- 金额与币种快照；
- `PREPAY_CREATED/CONFIRMING/SUCCESS/CLOSED/UNKNOWN` 状态；
- Provider 预支付标识；
- `paid_at`、通知处理结果；
...
4. 若已成功则幂等返回；
5. 要求时段仍由该订单锁定；
6. 支付转 `SUCCESS`，订单转 `CONFIRMED`，时段转 `BOOKED` 并清除临时锁截止字段；
7. 保存 Provider 交易号与支付时间并提交。

如果权威成功到达时场次已不再安全归属于原订单，不得覆盖其他订单。支付和订单进入 `PAYMENT_EXCEPTION`，保留完整异常记录，后续退款旅程处理资金；本切片通过测试保证正常超时流程不会提前释放，从源头降低该分支发生概率。
...
- 对 `CONFIRMING/UNKNOWN` 支付按持久化 `next_reconcile_at` 扫描；
- 使用 1、2、5、10、30 分钟退避并记录次数与最后错误；
- 服务重启后从数据库继续；
...
| `PAYMENT_ALREADY_CONFIRMED` | 200/409 | 刷新后进入已确认，不能再次付款 |
| `PAYMENT_CREATE_FAILED` | 502/503 | 保持待支付，允许同键重试 |
| `PAYMENT_CONFIRMING` | 202 | 显示确认中并继续查询 |
| `PAYMENT_EXCEPTION` | 409 | 明确异常、保持安全锁定并允许重新查询 |
