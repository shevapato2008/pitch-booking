...
平台创建邀请后只在首次 `201` 响应中得到一次原始令牌与小程序路径；数据库持久化 `SHA-256` 摘要，不保存可重放原文。相同幂等键的 `200` 重放只返回不含令牌的邀请元数据。若首次响应丢失，运营人员必须从列表确认权威状态，撤销该邀请并重新创建，不能从服务端恢复原始令牌。令牌使用密码学安全随机值，默认七天过期。
...
- 接受/提交/撤销先记录可恢复的本次 attempt，同 key 同请求重放原响应。创建是唯一例外：同 key 同请求仅重放不含 secret 的权威元数据；不同请求仍返回 `IDEMPOTENCY_KEY_REUSED`。
