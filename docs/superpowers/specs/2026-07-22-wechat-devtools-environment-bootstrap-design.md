...
2. Move a minimal design-preview Gallery shell into Task 6 before component visual decisions. Mount each component and its Fixture states there as it is designed, inspect it at 375px and 390px, and require a clean native compile/render before the Task 6 commit. Task 7 then expands that shell into the complete manifest-driven Gallery and Scenario Runner; it is not the first native preview.
...
7. A human records a redacted run-evidence result confirming that the development project compiled, rendered the native preview host at 375px and 390px, and showed no WXML/WXSS/Console errors. This evidence stays in the ignored run-evidence directory.

If the IDE is already serving another port, the preflight must fail as `WECHAT_PORT_MISMATCH`, print the safe remediation “quit Developer Tools, then rerun with one port,” and must not kill or reconfigure the user's IDE automatically. Other required stable failures are `WECHAT_CLI_INVALID`, `WECHAT_VERSION_UNAVAILABLE`, `WECHAT_APPID_REQUIRED`, `WECHAT_BUILD_FAILED`, `WECHAT_LOGIN_REQUIRED`, `WECHAT_OPEN_FAILED`, `WECHAT_AUTOMATION_FAILED`, and `WECHAT_NATIVE_INSPECTION_REQUIRED`.
...
- the plan contains the exact stable preflight failures, an explicit hard gate before Task 6, and a Task 6 native preview host; it no longer defers first CLI discovery or first native preview to Tasks 7–10;
- `node --test tests/llm-wiki.test.mjs tests/artifacts.test.mjs`, `npm run contract:validate`, `npm run lint`, `npm run typecheck`, and `npm test` all pass.
